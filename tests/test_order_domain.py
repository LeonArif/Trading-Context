...
def test_place_market_order():
    order = Order.place_market_order(
        user_id="user456", symbol="ETH/USDT", side=OrderSide.SELL, quantity=Decimal("5")
    )
...
def test_order_fill_zero_quantity_raises_error():
    ...
    with pytest.raises(
        InvalidQuantityException, match="Filled quantity must be greater than 0"
    ):
...
def test_order_validation_negative_quantity_raises_error():
    with pytest.raises(
        InvalidQuantityException, match="Quantity must be greater than 0"
    ):
        Order.place_limit_order(
            user_id="user123",
            symbol="BTC/USDT",
...
def test_order_validation_currency_mismatch_raises_error():
    ...
    with pytest.raises(
        OrderValidationException, match="Price currency EUR does not match"
    ):
        Order.create(
            user_id="user123",
            trading_pair=trading_pair,
